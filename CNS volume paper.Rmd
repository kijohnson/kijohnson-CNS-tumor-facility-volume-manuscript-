...
FVA_CNS2$Race_cat2<- factor(FVA_CNS2$Race_cat2,levels=c(0:4), 
                            labels=c("White",
                                     "Black",
...
FVA_CNS2$income<-factor(FVA_CNS2$income, levels=c(0:3), 
                        labels=c("Income Q1", 
                                 "Income Q2", 
                                 "Income Q3", 
                                 "Income Q4"))
...
FVA_CNS2$Surgery_AF<-factor(FVA_CNS2$Surgery_AF, levels=c(0:1),
                            labels=c("No surgery",
                                     "Surgery"))
...
FVA_CNS2$Surgery_TF<-factor(FVA_CNS2$Surgery_TF, levels=c(0:1),
                            labels=c("No surgery",
                                     "Surgery"))
...
FVA_CNS2$Rad_AF<-factor(FVA_CNS2$Rad_AF, levels=c(0:1),
...
FVA_CNS2$Chemo_AF<-factor(FVA_CNS2$Chemo_AF, levels=c(0:1),
...
```{r}
#The set of commented out code defines quartiles on an individual level. Need to redefine on a facility level because for adults Q4 only contains one facility.
# create Peds dataset (0-14 years)
peds_exclude1 <- exclude1 %>%
  filter(exclude1$AGE<15)

table(exclude1$YEAR_OF_DIAGNOSIS)

# create adolescent and adults data set (15+)
aa_exclude1 <- exclude1 %>%
  filter(exclude1$AGE>=15)

#calculate sum facility volume for each facility
...
test<-peds_facility_vol %>%
  filter(CNS_VOL>4)
hist(test$CNS_VOL)
hist(peds_facility_vol$CNS_VOL)
...
geom_vline(aes(xintercept=1.42857143), color="blue", linetype="dashed") +
geom_vline(aes(xintercept=3.35714286), color="blue", linetype="dashed") +
geom_vline(aes(xintercept=9.29285714), color="blue", linetype="dashed") +
geom_vline(aes(xintercept=14.50714286), color="blue", linetype="dashed") +
...
quantile(peds_facility_vol$CNS_VOL, probs = seq(.1, 1, by = .05))#note quantiles are at facility level

peds_CNS<-merge(peds_exclude1, peds_facility_vol, by="PUF_FACILITY_ID")


#make percentile categories
peds_CNS<-peds_CNS %>%
...
table(peds_CNS$vol_ptile)

peds_CNS$CNS_VOL_flag<-"peds"

peds_CNS$vol_ptile<-factor(peds_CNS$vol_ptile, levels=c(0:4), labels=c("<=70", ">70-80", ">80-90", ">90-95", ">95"))

#adult code
#calculate facility volume for each facility
aa_facility_vol<-aa_exclude1 %>%
  count(PUF_FACILITY_ID) %>%
  rename(CNS_VOL=n)%>%
  mutate(CNS_VOL=CNS_VOL/14)
quantile(aa_facility_vol$CNS_VOL, probs = seq(.1, 1, by = .05))
...
label(exclude3$educ)<-"Zip-code median percent no high school degree quartile 2012-2016"
label(exclude3$income)<-"Zip-code median income quartile 2012-2016"
...
```{r}
#Cox models with RSE to account for clustering
...
write.xlsx(Sup_table8, file="Sup_table8.xlsx", keepNA=TRUE, overwrite=TRUE)
```
...
# Cox models that account for clustering of patients in hospitals (models random effect separately), Not used
```{r}
#R code for Cox model with random coefficients (effect of shock varies across hospitals)
CoxRandom<-exclude3 %>%
  filter(age_cat=="0 to 14") 

mod1<-coxme(Surv(DX_LASTCONTACT_DEATH_MONTHS, dead)~1 + (1|PUF_FACILITY_ID), ties="efron", CoxRandom)
summary(mod1)
summary(exclude3$DX_LASTCONTACT_DEATH_MONTHS)

Model.frailty.gamma <- coxph(Surv(DX_LASTCONTACT_DEATH_MONTHS, dead) ~ vol_ptile + AGE  + Race_cat2 + educ +income +insurance + sex + CNStype + frailty(PUF_FACILITY_ID, distribution="gamma"), data=CoxRandom)
summary(Model.frailty.gamma)

Model.cluster <- coxph(Surv(DX_LASTCONTACT_DEATH_MONTHS, dead) ~ vol_ptile + AGE  + Race_cat2 + educ +income +insurance + sex + CNStype + cluster(PUF_FACILITY_ID), data=CoxRandom)
...
sheets<-list("Table2 (overall CPH)" = table2, 
             ...
             "Sup Table1 (EM tests)" = Sup_table1, 
             "Sup Table2 (cancer type CPH)" = Sup_table2, 
             "Sup Table3 (PH tests)" = Sup_table3, 
             "Sup Table4 (PH int)" = Sup_table4, 
             "Sup Table5 (PH int cantype)" = Sup_table5,
             "Sup Table6 (RMST unadj)" = Sup_table6,
             "Sup Table7 (RMST adj by type)" = Sup_table7,
             "Sup Table8 (Treatment)" = Sup_table8,
             "Sup Table9 (Year modification)" = Sup_table9)
